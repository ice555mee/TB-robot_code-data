...
path = "C:\work\Konthee\VISTEC\ML job\Sudo battery\MLClasiifiy\GCD/UnknownGCDall"
...
for f in files:
    ...
    #prediction
    image_path = f"C:\work\Konthee\VISTEC\ML job\Sudo battery\MLClasiifiy_Model2\GCD\UnknownGCDall/{f}"
    ...
    if score[0]==np.max(score) :
        PictureInt = "Bat"
        label = 0
        cv2.imwrite(f"C:\work\Konthee\VISTEC\ML job\Sudo battery\MLClasiifiy_Model2\GCD/OutputGCDall\{label}_{PictureInt}_0_{2*score[0]-0.5:.2f}_{f}.png",imgresize)
    elif score[1]==np.max(score) :
        PictureInt = "Pse"
        label = 1
        cv2.imwrite(f"C:\work\Konthee\VISTEC\ML job\Sudo battery\MLClasiifiy_Model2\GCD/OutputGCDall\{label}_{PictureInt}_0_{2*score[1]-0.5:.2f}_{f}.png",imgresize)  
