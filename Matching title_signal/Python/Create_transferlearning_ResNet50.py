...
#Prepare data
import pathlib
datadir = "C:\work\Konthee\VISTEC\ML job\Sudo battery\TestMLProgram\PictureExt" #path ของ dataset
...
prediction_layer=tf.keras.layers.Dense(units=num_classes,activation="softmax")(global_average_layer)
...
##Train 
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learningRate),loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),metrics=['accuracy'])
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learningRate/10),loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),metrics=['accuracy'])
